...
function rbfcolor(image_name, maxsize , controlmap, callback, grid_param){
	...
	imageObj.onload = function(){
		...
		{
	    	img_width = maxsize;
        	img_height = Math.ceil(img_width/imageObj.width*imageObj.height);
		}else
		{
	    	img_height = maxsize;
        	img_width = Math.ceil(img_height/imageObj.height*imageObj.width);
		}*/
	...
	//Kmeans
	this.calcKMeans = function(center_num)
	{
		...
		{
			Center[i] = D[ Math.floor(Math.random() * this.img_area) ];
		}
	...
	this.grid_calcKMeans = function(center_num)
	{
		...
		for (var i = 0;i < center_num;i++)
		{
			Center[i] = D[ Math.floor(Math.random() * this.grid_size) ];
		}
	...
	this.fast_calcKMeans = function(center_num, ptLimit)
	{
		...
		{
			var index = Math.floor( i / (ptLimit-1) * (this.img_area-1)  );
		...
		for (var i = 0;i < center_num;i++)
		{
			Center[i] = D[ Math.floor(Math.random() * ptLimit) ];
		}
		...
		{
			var ps = LAB2RGB(Center[i]);
			res[i*3+0] = Math.max( Math.min( 1.0,ps[0]/255 ) );
			res[i*3+1] = Math.max( Math.min( 1.0,ps[1]/255 ) );
			res[i*3+2] = Math.max( Math.min( 1.0,ps[2]/255 ) );
		}
	...
	this.adapted_calcKMeans = function(grid_size)
	{
		//initialize
		...
		for (var i = 0;i < center_num;i++)
		{
			var k = Math.floor(Math.random() * this.img_area);
			Center[i] = RGB2LAB([this.ori_r[k],this.ori_g[k],this.ori_b[k]])
			
			/*
			var k = Math.floor(Math.random() * pt_size);
		...
		while(true)
		{
			...
			for(var k=0;k<pt_size;k++)
			{
				...
				for(var i=0;i < center_num;i++)
				{
					//weight_sum = Math.max(weight_sum , precompute_pinv[i]);
					
					weight_sum =weight_sum + precompute_pinv[i];
					//weight_sum =weight_sum + Math.max(precompute_pinv[i],0.0);
				}
		...
		{
			var ps = LAB2RGB(Center[i]);
			res[i*3+0] = Math.min(Math.max(ps[0]/255,0.0),1.0);
			res[i*3+1] = Math.min(Math.max(ps[1]/255,0.0),1.0);
			res[i*3+2] = Math.min(Math.max(ps[2]/255,0.0),1.0);
		}
...
this.gridacc_kmeans = function(center_num)
      {
        ...
        console.log(this.ori_r[0]);
        console.log(step_size);
          for( var i = 0;i < this.img_area;i++ )
          {
              var p = RGB2LAB([ this.ori_r[i], this.ori_g[i], 
this.ori_b[i] ]);
...
(D[i][2]-Center[j][2])*(D[i][2]-Center[j][2]);
                  ...
                  sumD[min_id][0] = sumD[min_id][0] + sample_cnt[i] * 
D[i][0];
                  sumD[min_id][1] = sumD[min_id][1] + sample_cnt[i] * 
D[i][1];
                  sumD[min_id][2] = sumD[min_id][2] + sample_cnt[i] * 
D[i][2];
              }
